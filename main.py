...
# ─── Legacy cache purge (privacy fix 2026-08-11) ────────────────────
# The file cache was retired to match our 1-hour deletion promise. On
# every boot, wipe any leftover cache directory a previous deploy may
# have created — belt-and-braces so no user file bytes survive a
# rollout regardless of what shape the old cache had on disk.
_LEGACY_CACHE_DIRS = [
    os.environ.get("PDF_CACHE_DIR", "./cache"),
    "./cache",  # historic default even when the env var was set elsewhere
]
for _dir in {os.path.abspath(p) for p in _LEGACY_CACHE_DIRS}:
